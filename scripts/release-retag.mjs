 ...
 *
 * 用法：
 *   npm run release:retag              # 使用 src-tauri/tauri.conf.json 中的 version
 *   npm run release:retag -- 0.0.2     # 指定版本号
 *
...
const version = arg || JSON.parse(readFileSync('src-tauri/tauri.conf.json', 'utf8')).version;
const tag = `v${version}`;
