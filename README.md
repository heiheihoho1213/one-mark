...
> 开发环境：
- macOS 14.8.4，Apple M1 Mac mini 2020
- Node.js v24.14.0
- Rust 1.96.0
> 打包环境：macOS 14.8.4，Apple M1 Mac mini 2020
> 适配平台：Windows 7+、macOS 10.15+
...
npm run release              # 按 tauri.conf.json 的 version 打 tag 并推送（推荐）
npm run release -- "1.0.1"   # 指定版本；0.0.x 必须加引号，否则 zsh 会变成 0.02
npm run release:retag        # 已有 tag 时重发（自动读 tauri.conf.json）
